...
- 이상의 코드에서 중요한 점은 리렌더링이 일어나는 시점에 해당합니다. 코드를 붙여넣은 다음 브라우저에서 실행하더라도 해당 부분을 검증하는 것은 어려우므로, 꼭 리렌덜링이 일어나는 시점은

- 전체 상태가 업데이트가 끝난 뒤

- 라는 점을 명심하시면 좋겠습니다.

cf) 기껏 onClick={() => setCount(preCount => preCount + 1)} 이라고 화살포 함수 쓰라고 해놓고 저기서는 왜 그냥 onClick={increment}라고 썼는지에 대해서 의문이 생기신다면 여러분이 착실히 학습하고 있다는 겁니다.
...
const increment = () => {
  flushSync( () => {
    setCount(count + 1);  //하는 경우 fluchSync 내에 있는 부분은 일괄 업데이트 처리가 아니라 바로 이루어짐
  })
}
```
...
App7.jsx에서 useEffect는 처음 새로고침을 했을 대 2 번 실행되고, 버튼을 누를 때 마다 한 번씩  실행이 됐습니다.
...
- 메모리 누수를 발견하기 위해서 : 선능향상관점
- 클린업(cleanup) : unmount가 제대로 일어나는지를 확인
- useEffect의  첫 번재 argument인 콜백 함수가 의도한 대로 제대로 실행되는지를 테스트 하기 위해
...
[count]를 두 번째 argument로 삽입했을 경우, count의 상태값이 바뀔 때마다 렌더링이 일어압니다.
...
정의에서처럼 useEccect는 보조 작업을 사용할 때 callback 함수를 첫 번째 argument로 사용합니다.

보조 작업의 예시 -> 구독, 타이머 또는 예기치않은 작동을 방지하기 위해 정리해야 하는 리소스를 설정할때(cleanup) 유용합니다.

정리 기능은 페이지에서 컴포넌트를 제고(또는 마운트 해제)한 후에도 실행이 됩니다.
...
: useRef 훅인 DOM 노드에 접근하는 데 이용할 수 있는 변경이 가능한 ref 객체를 반환.
...
- p 태그와 button 태그를 사용하여 클릭할 때 마다 p 태그의 바귄 카운트 값이 나올 수 있오록 작성하시오.

## Context API

- 컴포넌트 트기가 깊고 복잡하다면 프롭을 이용하여 데이터를 전달하는 것이 번거로울 수 있음.
