...
- invalidateQueryies() : 캐시 내 쿼리를 무효화하는 데 사용. 만약 캐시에서 쿼리가 무효화되면 해당 쿼리는 다시 데이터를 가져올 것입니다.
...
const mutation = useMutation({
  mutatioin: (newCar) => {
    return axios.post(`api/cars`, newCar0;)
  },
  onError: (error, variables, context) => {
    // 변경 오류 관련 로직
  },
...
: React를 이용한 네트워킹.
1. 비동기 네트워크 호출을 더쉽게 구현하는 프로미스(콜백 함수의 발전형)
...
서드파티 컴포넌트 설치 명령어
```cmd
npmm install 컴포넌트_이름

그래서 저희는 
npminstall react-date-picker
```

npm install react-date-picker로 설치하면 가장 최신 버전 설치됩니다

npm install react-date-picker@4
일 경우에 저희가 vite profect 만들 때처럼 버전을 명시하는 것이 가능합니다.
...
프로젝트 루트 이렉토리에서 오래된 컴포넌트를 확인하는 명령어

```cmd
npm outdated
```
츨력이 비어있으면 모든 컴포넌트가 최신이라는 의미
한꺼번에 업데이트하려면
```cmd
npm update
```
