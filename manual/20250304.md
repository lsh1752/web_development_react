...
## 객체 / 각 필드 형태의 차이
1. hsndleSubmit 부분에서 스프레드 문법의 사용유뮤
...
- 타입스크립트를 이용할 때 코드 가독성이 향상되고 코드 자체의 문서화가 용이함 때 코드 가독서잉 향상되고 코드 자체의 문서화가 용이함

## 환경 설정

로컬 상호아에서 타입스크립트를 코딩하려면 npm을 이용하여 컴퓨터에 타입스크립트 컴파일러를 설치해야 합니다. 근데 vite에는 타입스크립트 지원이 내장되어 있기 때문에 리액트 프로젝트에서는 설치할 필요가 없습니다. 타입스크립트는 일반 자바스크립트로 트랜스파일 한 다음, 자바스크립트 엔진에서 실행이 가능합니다.
...
### 공통 타입 
- 타입스크립트는 변수를 초기화할 때 변수의 타입을 자동으로 정의하는데, 이를 '타입 추론'이라고 한다. 그래서 특정 타입으로 정의해 놓은 다음에, 다른 타입으로 재할당을 시도할 경우 오류가 발생합니다.ㅣ

```js
let message = 'Lello, TypeScript';
console.log(message)
message = 5;
console.log(message)
```
...
* 타입스크립트는 any 타입(JS에서 있는)도 제공합니다. any 타입을 이용하여 변수를 정의하는 경우 TS는 해당 변수에 대해 타입 검사 또는 추론을 수행하지 않습니다. 타입스크킵트의 정적 타입 분석을 무효화하기 때문에 가능한한 any 타입은 이ㅛㅇ하지 않는 것이 권장됩니다.
...
- 그래서 첫 번째 ts 수업에서의 부분과 같이 타입 추론을 통해서 id: number로 ts 상에서 결로닝 났을 경우 student.id = '이번'; 과 같은 방식으로 추론에서 어긋나는 자료형으로 재대입을 한 경우 ts에서는 오류 메시지를 출력하게 됩니다.
...
```
- interface / type은 객체의 모양을 설명하는 키원드로 객체를 생성할 때 특정 property에 들어갈 value의 자료형을 강제하는 역할을 한다. type과 interface는 상당히 유사하며 보통 둘 중 아무거나 사용해도 무방합니다.

- 그래서 이상의 interface 혹은 type을 ㅣ용한 객체 생성 방식
```ts
...
이상이 코드에서는 문자열 혹은 숫자를 포함할 수 있느 자료형을 정의했습니다.
...
- 그래서 쉼표로 작성했을 때 정상적으로 동작하지만 공식 스타일 가이트에서는 ;을 권장합니다.
