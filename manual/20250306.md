...
axios라이브러리에는 JSON 데이터의 자동 변환가 같은 여러가지 이점이 있기 때문에 json() 메서드가 필요하지 않습니다. 그래서 어제 배운 fetch 관련해서의 코드를  axios로 작성했을 때는
...
2. weather 객체를 만든 다음 거기에 property로 온도 / 설명 / 날시아이콘을 집어넣는 방법

https://api.openweathermap.org/data/2.5/weather?q=Seoul&units=Metric&APIkey=cb4b193fbfcbf300d31cb3358539a4f1
...
그 다음에 터미널에서ctrl + c 눌러서 서버 종료 하신 다음에
...
개발자 도구 상에서 Network 탭에서 200 OK인 것도 확인했ㅅ브니다.
...
필용한 날씨 설몀ㅇ, 아이콘, 기온을 가지고 와서 useState를 통해 생성한 객체에 집어넣은 후,
...
// function App 빼놓고 복사해왔습니다
  const [ keyword, setDeyword ] = useState('');
...
또한 id 속서이 있어서 고유값으로 쓸 예정입니다. 이를 테이블 행의 키로 사용할겁니다.
...
주의해서 봐야할 점은 삼항연산자가 응용됐습니다. 근데 끝나느 시점을 확인하는 거싱 쉽지 않다.

검색 결과가 없으면 repodata.length === 0 이기 때문에 표시할 정보가 없다고 뜰 것이고 뒷부분에 : 뒤에는 검색 겨로가가 하나라도 있으면 그것을 table 형태로 표시해주게 됩니다.
...
* 참고 : github API는 요청횟수 제한이 적용돼서 너무 많이 써먹으면 안됩니다. 1분에 10회 요청 제한이 있어서 그 이상 검색하고 나면 1 분 기다리셔야 합니다.
...
20250307에는 useEffect 정리 함수를 활용하여 경쟁 조건을 피하도록 작성하는 방버블 수업할겁니다.
