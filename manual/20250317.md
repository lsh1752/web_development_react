...
: Props Drilling은 데이터를 사용하지 않는 App의 직계 자식 컴포넌트 또한 props를 전달받아야한다는 점입니다. Main 컴포넌트는 자식 컴포너느에 props를 보내주기만 할 뿐이고, 직접 필요호 하지 않기 때문에 메모리적으로 비효율적입니다.
