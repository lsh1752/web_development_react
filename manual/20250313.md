...
data.js의 상수인 CORE_CONCEPTS를 확인했을 때, 갹 element들이 image, description, title이라는 속성을 가진 객체들이라는 점을 확인했을 때,
...
3. 배열에서의 element 추가
: tmvmfpem ansqjqdmf tkdydgkf Eo rlwhs qodufdml dkvdlsk enldp tofhdns dythfmf cnrkgkrl dyddl
...
3. 간단하게 배열 및 객체를 조작 가능(특정 메서드를 외워서 쓸 필요성이 낮아짐)
...
1. App.jsx에서 function CoreCoincepts에 해당하는 부분을 전부 잘라내서 CoreConcepts.jsx로 이동 -> function 앞에 export default 썼습니다.

2. index.css에서 #core-concepts로 디어있는 부분들 다 잘라내서 CoreConcepts.css로 이동합니다.
...
```jsx
import CoreConcepts from"./compoonents/CoreConcepts/CoreConcepts";
```
...
즉, 부모 컴포넌트가 <Component> ... </Component> 형태로 JSX를 감싸면, 이 감사진 내용이 컴포넌트의 children prop으로 전달.

### children prop의 기본 동작

React 상에서 컴포넌트가 중첩 구조를 가질때, 내부에 포함된 요소는 자동으로 children으로 전달됨.
...
export default function TabButton({children}) {
  ...
  // 원래 JS에서는 이상과 같은 방식으로 명령령으로 작성했었지만,
...
이상의 결과로 렌더링될 때 handleSelect함수의 결과값인 이잉이잉이 콘솔에 출력됩니다.

-> 즉 버튼을 눌렀을 때 이잉이잉이 출력되지 않는다는 점이 문제
