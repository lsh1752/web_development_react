...
// 즉 여러분들은 자바 배울 때 객체 지향 하면서 파일 넘나들면서 오류 수정했던 부분들을 JavaScript에서 잠깐 놓아두셧지만(index에 sxript 추가했냐 정도만 하셨지만) 이제는 React 상황에서 여러 파일을 검증해서 오류를 밝혀내야 할 필요가 있습니다.

// main.jsx -> 상위 컴포넌트로서 키-값 쌍을 정의하고(user라는 키와 Ahn이라는 값) 하위 컴포넌트인 Hello로 전달

// 그 때 props = {user: 'Ahn'}으로 대입되면, 이후 <h1>Hello {props.user}</h1>은 Hello Ahn으로 출력되게 됩니다.
...
// 이상의 코드가 의미하는 바는 : props 내위 복수의 키-값 쌍(properties)가 있을 수 있음을 보여줍니다.
